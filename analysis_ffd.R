### The following code reproduces the "Full Factorial Design" analysis with
### first- and second-order meta-models described in the paper "Using design of
### experiments with discrete event simulation in operational research: a review"
### by Gjerloev et al.
...
### Setting FFD analysis parameters ################################################
n_levels <- 3
...
basic_design <- data.frame(
  alpha = rep(c(rep(min_alpha,n_levels^0),rep(centre_alpha,n_levels^0),rep(max_alpha,n_levels^0)),n_levels^3),
  ecu_beds = rep(c(rep(min_ecu_beds,n_levels^2),rep(centre_ecu_beds,n_levels^2),rep(max_ecu_beds,n_levels^2)),n_levels^1),
  beta = rep(c(rep(min_beta,n_levels^1),rep(centre_beta,n_levels^1),rep(max_beta,n_levels^1)),n_levels^2),
  arr_rate = rep(c(rep(min_arr_rate,n_levels^3),rep(centre_arr_rate,n_levels^3),rep(max_arr_rate,n_levels^3)),n_levels^0)
)
rownames(basic_design) <- paste("comb",1:nrow(basic_design),sep='_')



### Running simulations for FFD analysis ##########################################
...
saveRDS(simulation_results,file=paste("data/",analysisID,"/output/simulation_results_ffd.rds",sep=''))

# Uploading raw simulation results - if needed, to avoid recomputing them
#simulation_results <- readRDS(paste("data/",analysisID,"/output/simulation_results_ffd.rds",sep=''))
...
## Fitting simulation data with first-order meta-models ################

wait_ECU <- rsm(wait_ECU ~ FO(x1,x2,x3,x4), data = coded_design)
sink(paste("data/",analysisID,"/output/wait_ECU_FO_summary.txt",sep=''))
print(summary(wait_ECU))
sink()
png(paste("data/",analysisID,"/output/wait_ECU_FO.png",sep=''),height=800,width=1000)
par(mfrow = c(2,3), cex=1.5)
contour(wait_ECU, ~ x1 + x2 + x3 + x4, image = TRUE, labcex=1.5, main = "Waiting ECU")
dev.off()

wait_ICU <- rsm(wait_ICU ~ FO(x1,x2,x3,x4), data = coded_design)
sink(paste("data/",analysisID,"/output/wait_ICU_FO_summary.txt",sep=''))
print(summary(wait_ICU))
sink()
png(paste("data/",analysisID,"/output/wait_ICU_FO.png",sep=''),height=800,width=1000)
par(mfrow = c(2,3), cex=1.5)
contour(wait_ICU, ~ x1 + x2 + x3 + x4, image = TRUE, labcex=1.5, main = "Waiting ITU")
dev.off()

overall_wait <- rsm(overall_wait ~ FO(x1,x2,x3,x4), data = coded_design)
sink(paste("data/",analysisID,"/output/overall_wait_FO_summary.txt",sep=''))
print(summary(overall_wait))
sink()
png(paste("data/",analysisID,"/output/overall_wait_FO.png",sep=''),height=800,width=1000)
par(mfrow = c(2,3), cex=1.5)
...
util_ECU <- rsm(util_ECU ~ FO(x1,x2,x3,x4), data = coded_design)
sink(paste("data/",analysisID,"/output/util_ECU_FO_summary.txt",sep=''))
print(summary(util_ECU))
sink()
png(paste("data/",analysisID,"/output/util_ECU_FO.png",sep=''),height=800,width=1000)
par(mfrow = c(2,3), cex=1.5)
contour(util_ECU, ~ x1 + x2 + x3 + x4, image = TRUE, labcex=1.5, main = "Utilisation ECU")
dev.off()

util_ICU <- rsm(util_ICU ~ FO(x1,x2,x3,x4), data = coded_design)
sink(paste("data/",analysisID,"/output/util_ICU_FO_summary.txt",sep=''))
print(summary(util_ICU))
sink()
png(paste("data/",analysisID,"/output/util_ICU_FO.png",sep=''),height=800,width=1000)
par(mfrow = c(2,3), cex=1.5)
contour(util_ICU, ~ x1 + x2 + x3 + x4, image = TRUE, labcex=1.5, main = "Utilisation ITU")
dev.off()

overall_util <- rsm(overall_util ~ FO(x1,x2,x3,x4), data = coded_design)
sink(paste("data/",analysisID,"/output/overall_util_FO_summary.txt",sep=''))
print(summary(overall_util))
sink()
png(paste("data/",analysisID,"/output/overall_util_FO.png",sep=''),height=800,width=1000)
par(mfrow = c(2,3), cex=1.5)
...
costs_ECU <- rsm(costs_ECU ~ FO(x1,x2,x3,x4), data = coded_design)
sink(paste("data/",analysisID,"/output/costs_ECU_FO_summary.txt",sep=''))
print(summary(costs_ECU))
sink()
png(paste("data/",analysisID,"/output/costs_ECU_FO.png",sep=''),height=800,width=1000)
par(mfrow = c(2,3), cex=1.5)
contour(costs_ECU, ~ x1 + x2 + x3 + x4, image = TRUE, labcex=1.5, main = "Costs ECU")
dev.off()

costs_ICU <- rsm(costs_ICU ~ FO(x1,x2,x3,x4), data = coded_design)
sink(paste("data/",analysisID,"/output/costs_ICU_FO_summary.txt",sep=''))
print(summary(costs_ICU))
sink()
png(paste("data/",analysisID,"/output/costs_ICU_FO.png",sep=''),height=800,width=1000)
par(mfrow = c(2,3), cex=1.5)
contour(costs_ICU, ~ x1 + x2 + x3 + x4, image = TRUE, labcex=1.5, main = "Costs ITU")
dev.off()

overall_costs <- rsm(overall_costs ~ FO(x1,x2,x3,x4), data = coded_design)
sink(paste("data/",analysisID,"/output/overall_costs_FO_summary.txt",sep=''))
print(summary(overall_costs))
sink()
png(paste("data/",analysisID,"/output/overall_costs_FO.png",sep=''),height=800,width=1000)
par(mfrow = c(2,3), cex=1.5)
...
# Applying fitted first-order meta-models to arbitrarily chosen scenarios
...
for(i in names(wait_model)){
  png(paste("data/",analysisID,"/output/pred_wait_FO_alpha_",i,".png",sep=''),height=1000,width=1000,pointsize=36)
...
col_wait <- c(colorRampPalette(c("#32CD32", "#A3A3A3"))(length(lev_wait)))
for(i in names(wait_model)){
  png(paste("data/",analysisID,"/output/pred_wait_SO_alpha_",i,".png",sep=''),height=1000,width=1000,pointsize=36)
  filled.contour(ecu_beds_test,arr_rate_test,wait_model[[i]], levels=lev_wait,col=col_wait, plot.axes = {
    axis(1);
    axis(2);
    contour(ecu_beds_test,arr_rate_test,wait_model[[i]],levels=lev_wait, add = TRUE, lwd = 1)
